...
class Structure:
    ...
    def bsp_grid(self, grid, room_count, offset_x=0, offset_y=0):
        ...
        split_width = random.randint(
            (width // 3.5), width - 2) if width >= 3 else width // 2
        split_height = random.randint(
            (height // 3.5), height - 2) if height >= 3 else height // 2
        ...
        if split_count <= 0:
            ...
            if width >= height:  # vertical split
                ...
                self.rooms.append(
                    Room(left_x, left_y, left_size[0], left_size[1]))
                self.rooms.append(
                    Room(right_x, right_y, right_size[0], right_size[1]))
                ...
                self.rooms.append(
                    Room(bottom_x, bottom_y, bottom_size[0], bottom_size[1]))
            return
        ...
        if width >= height:  # vertical split
            ...
            if len(left[0]) > len(right[0]):
                ...
                self.bsp_grid(right, split_count, offset_x +
                              split_width + 1, offset_y)
            ...
            for i in range(width):
                ...
                left.append([(i + offset_x, y + top_y)
                            for y in range(top_size[1])])
                right.append([(i + offset_x, y + bottom_y)
                             for y in range(bottom_size[1])])

            # these 2 lines transpose the above matrices so they resemble the grid
            left = [[left[j][i]
                     for j in range(len(left))] for i in range(len(left[0]))]
            right = [[right[j][i]
                      for j in range(len(right))] for i in range(len(right[0]))]
    ...
    def build_space_graph(self):
        ...
        for i in room_combinations:
            ...
            if b_min_x <= a_max_x and b_max_x >= a_min_x and b_min_y <= a_max_y and b_max_y >= a_min_y:
                ...
                x_overlap = [x for x in list(iter.chain(
                    range(a_min_x, a_max_x+1))) if x in list(iter.chain(range(b_min_x, b_max_x+1)))]
                y_overlap = [y for y in list(iter.chain(
                    range(a_min_y, a_max_y+1))) if y in list(iter.chain(range(b_min_y, b_max_y+1)))]
        ...
        print(
            f"largest room: {space_graph.get_node_data(largest_room_index)} // {current_room_size} cells")
        print(
            f"larget room neighbors: {list(space_graph.neighbors(largest_room_index))}")
...
print("hello world! its me from Lazy Vim")
